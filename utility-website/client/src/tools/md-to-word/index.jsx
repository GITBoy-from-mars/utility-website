  ...
  const handleProcess = async () => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        a.href = url;
        a.download = files[i].name.replace(/\.(md|markdown|txt)$/i, '.docx');
        a.click();
  ...
  return (
    <ToolPageWrapper meta={meta}>
      <div className="converter-tool">
        <FileUploader accept=".md,.markdown,.txt" multiple={true} maxSizeMB={10} onFilesSelected={setFiles} label="Drop Markdown files here or click to browse" sublabel="Supports .md, .markdown, .txt · Max 10MB per file" />
        {files.length > 0 && (
