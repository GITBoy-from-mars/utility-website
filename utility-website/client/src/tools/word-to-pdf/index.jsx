  ...
  const handleProcess = async () => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        const res = await fetch(`${import.meta.env.VITE_API_URL}/api/tools/word-to-pdf/convert`, { method: 'POST', body: formData });
        if (!res.ok) throw new Error('Conversion failed');
        const blob = await res.blob();
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = files[i].name.replace(/\.(docx?|doc)$/i, '.pdf');
        a.click();
  ...
  return (
    <ToolPageWrapper meta={meta}>
      <div className="converter-tool">
        <FileUploader accept=".docx,.doc" multiple={true} maxSizeMB={50} onFilesSelected={setFiles} label="Drop Word files here or click to browse" sublabel="Supports .docx and .doc · Max 50MB per file" />
        {files.length > 0 && (
          <button onClick={handleProcess} disabled={processing} className="btn btn-primary btn-lg converter-btn">
            ...
            {processing ? 'Converting...' : `Convert ${files.length} File${files.length > 1 ? 's' : ''} to PDF`}
          </button>
        )}
